...
export default {
  ...
  investmentsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.amount);
  },
  investmentsOneListFormatter(val) {
    if (!val) return '';
    return val.amount;
  },
  investmentsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.amount };
    });
  },
  investmentsOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.amount, id: val.id };
  },

  startupsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.name);
  },
  startupsOneListFormatter(val) {
    if (!val) return '';
    return val.name;
  },
  startupsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.name };
    });
  },
  startupsOneListFormatterEdit(val) {
    if (!val) return '';
